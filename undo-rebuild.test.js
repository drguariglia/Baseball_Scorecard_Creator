...
assert.ok(html.indexOf('baseball-data.js?v=27.2') < html.indexOf('app.js?v=27.2-continuous-autosave'), 'baseball-data.js must load before app.js');
assert.match(html, /id="connectionBadge"/, 'connection status badge must be present');
assert.match(worker, /guariglia-scorecard-v27-2-continuous-autosave/, 'service worker cache must be bumped');
